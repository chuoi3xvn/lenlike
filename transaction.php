...
if($mang == 'CX')
{
$tong = mysql_result(mysql_query("SELECT COUNT(*) FROM `TheCao_CamXuc` WHERE `mathe` ='".$sopin."' && `seri` ='".$seri."'"), 0); 
//echo $tong; exit;
if($tong == 0)
{
echo '<script>alert("['.date("H:i:s").'] Thẻ Sai Hoặc Chưa Được Phát Hành");
	
	
	 window.location = "http://camxuc.mobi/naptien.php"
	</script>';	
}
else
{
$info = mysql_fetch_array(mysql_query("SELECT * FROM `TheCao_CamXuc` WHERE `mathe` ='".$sopin."' && `seri` ='".$seri."' LIMIT 1"));
mysql_query("UPDATE `users` SET `sodu` = `sodu` + '".$info[menhgia]."' WHERE `username`  ='".$user."'");
mysql_query("DELETE FROM `TheCao_CamXuc` WHERE id='" . mysql_real_escape_string($info[id]) . "'");
echo '<script>alert("['.date("H:i:s").'] Bạn đã thanh toán thành công thẻ CamXucMobi mệnh giá '.$info[menhgia].' VNĐ");
	
	
	 window.location = "http://camxuc.mobi"
	</script>';
}
exit;
}
...
$merchant_id = '27129';
//Api username 
$api_username = 'camxucmobi';
//Api Pwd d
$api_password = 'M5jB5PfkpD5yscm8AUu7';
//Mã TransactionId 
$transaction_id = time();
//mat khau di kem ma website dang kí trên B?o Kim
$secure_code = 'aabc9dbb6c6995fa';
...
if($status==200){
	...
	mysql_query("UPDATE `users` SET `sodu` = `sodu` + '".$xu."' WHERE `username`  ='".$user."'");
	...
	fwrite($fh,"\r\n");
	fclose($fh);
      echo '<script>alert("['.date("H:i:s").'] Bạn đã thanh toán thành công thẻ '.$ten.' mệnh giá '.$amount.'");
